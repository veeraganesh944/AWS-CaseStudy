...
function( e ) {
  ...
  $.ajax( {
    url: 'http://65.1.108.118:5000/upload-image',
    type: 'POST',
...
function readData(){
    $.get("https://fdpurq55gj.execute-api.ap-south-1.amazonaws.com/ProdTodo/tasks",function(data){
        ...
        for(let x in data.tasks){
           ...
           code+="<h5> Document :: <a href='http://65.1.108.118:5000/download-file/"+data.tasks[x].docname+"'>"+data.tasks[x].docname+"</a><h5>"
           code+="<h5>Image::<h5>"+"<img src='http://65.1.108.118:5000/download-file/"+data.tasks[x].imagename+"'>"
           code+="</li>"
        }
        code+="</ul>"
        $("#container").html(code)
